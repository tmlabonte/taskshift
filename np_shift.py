...
def plot(args, results):
    ...
    # plot_helper(
    #     ["y_diff_norm", "y_diff_norm_scale"],
    #     ["y diff", "y diff scaled"],
    #     "y",
    # )
def experiment_trial(args, results, idx, d):
    ...
    if args.cov_type == "isotropic":
        ...
        # for j in range(min(args.spiked_num, d)):
            # cov[j, j] = 100 * d #args.spiked_size
        cov[0][0] = d * 100
        cov = cov / (d*100)
        # cov = cov / (d * 100)
    elif args.cov_type == "poly":
    ...
    X_test = rng.multivariate_normal(np.zeros(d), cov, n).T
    ...
    M = X @ np.linalg.inv(X.T @ X)
    # temp_norm = np.linalg.norm(M.T @ y)

    theta_tilde = M @ y_tilde
    theta_hat = M @ y

    theta_tilde_norm = np.linalg.norm(theta_tilde)
    theta_hat_norm = np.linalg.norm(theta_hat)
    # y_diff_norm = np.linalg.norm(y_tilde - y)
    # y_diff_norm2 = np.linalg.norm(y_tilde - y / temp_norm)
    ...
    # results["y_diff_norm"][idx].append(y_diff_norm)
    # results["y_diff_norm_scale"][idx].append(y_diff_norm2)
...
def experiment(args):
    ...
    results = {
        ...
        # "y_diff_norm": [[] for _ in range(len(ds))],
        # "y_diff_norm_scale": [[] for _ in range(len(ds))],
    } 
